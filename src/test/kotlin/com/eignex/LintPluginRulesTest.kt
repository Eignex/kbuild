...
class LintPluginRulesTest {
    ...
    @Test
    fun `an undocumented public class is reported by detektMain`(@TempDir dir: File) {
        val probe = lintProbe(dir)
    ...
    @Test
    fun `a KDoc sentence without terminating punctuation is reported by detektMain`(@TempDir dir: File) {
        val probe = lintProbe(dir)
        probe.write(
    ...
    @Test
    fun `a deprecated KDoc block tag is reported by detektMain`(@TempDir dir: File) {
        val probe = lintProbe(dir)
        probe.write(
    ...
    fun `detekt regenerates its config when the configuration cache is reused after build is cleared`(
        ...
        probe.build("detektMain", "--configuration-cache")
        ...
        val result = probe.build("detektMain", "--configuration-cache")
        ...
        result.assertOutcome(":detektMain", TaskOutcome.SUCCESS)
    ...
    fun `lint tasks can be disabled`(@TempDir dir: File) {
        val result = lintProbe(dir, "eignexLint { enabled.set(false) }").build("detektMain")

        result.assertOutcome(":detektMain", TaskOutcome.SKIPPED)
    }

    @Test
    fun `every source file is analysed by exactly one detekt task`(@TempDir dir: File) {
        val probe = lintProbe(dir)
        probe.write("src/main/kotlin/Clean.kt", "internal class Clean\n")

        val result = probe.build("check")

        result.assertOutcome(":detekt", TaskOutcome.SKIPPED)
        result.assertOutcome(":detektMainSourceSet", TaskOutcome.SKIPPED)
        result.assertOutcome(":detektMain", TaskOutcome.SUCCESS)
    }

    @Test
    fun `every source file is analysed by exactly one detekt task on multiplatform`(@TempDir dir: File) {
        val result = kmpProbe(dir).build("detekt", "detektCommonMainSourceSet")

        result.assertOutcome(":detekt", TaskOutcome.SKIPPED)
        result.assertOutcome(":detektCommonMainSourceSet", TaskOutcome.SUCCESS)
    }
